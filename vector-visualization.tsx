...
import { projectVectorsTo3D } from "./utils/dimension-reduction"
import { analyzeMetadataFields, getColorMapping, getItemColor } from "./utils/data-analysis"
import type { ProcessedVectorItem, FieldMetadata } from "./types/vector-data"
...
function Scene({ numPoints, colorField, onFieldsAnalyzed, onColorMappingUpdate }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        // Fetch raw vector data from the service
        const fetchedData = await mockDataService.fetchVectorData({
          limit: numPoints,
          dimensions: 100,
        })
